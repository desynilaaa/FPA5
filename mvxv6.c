...
int main(int argc, char *argv[])
{
	...
	//menentukan keadaan folder sumber file
	struct stat st;
	fstat(fd_src, &st);
	if (st.type == T_DIR)
	{
	 ...
	 //menentukan parameter kedua "/", jika iya, makas isi
	char com[128] = {};
	strcpy(com, argv[2]);
	...
	if (argv[2][len2-1] == '/')
	{
		//temukan nama filenya di argv[1]
		int i = len1 - 1;
		for (; i >= 0; i--)
			if (argv[1][i] == '/')
				break;
...
char* fmtname(char *path)
{
	...
	// Temukan karakter pertama setelah garis miring terakhir.
	for(p=path+strlen(path); p >= path && *p != '/'; p--)
		;
	p++;
	
	// Return blank-padded name. // Kembalikan nama berlapis kosong.
	if(strlen(p) >= DIRSIZ)
		return p;
